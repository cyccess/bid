...
namespace OAuth.Service
{
    public class UserService : IUserService
    {
        ...
        public UserDto Login(string username, string password)
        {
            ...
            {
                var userDto = Mapper.Map<User, UserDto>(entity);
                userDto.IsEnabled = userDto.Status == 1;
                return userDto;
            }
        ...
        public bool CheckOldPassword(int uid, string oldPwd)
        {
            string password = EncryptHelper.Encrypt(oldPwd);

            var result = _repo.GetAll<User>().Any(u => u.Id == uid && u.Password == password);
