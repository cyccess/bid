+(function ($, window) {
	...
	User.fn = User.prototype = {
		...
		show: function () {
			...
			$.post("/mode/get", that.options, function (data) {
				that.data = data;
				that.$sidebarLoading.hide();
				that.$sidebarContent.html($("#mode_edit").tmpl(data));
				...
				$("#form1").Validform({
					...
					beforeSubmit: function () {
						$("#form1").ajaxSubmit({
							...
							url: "/mode/edit/?timestamp=" + new Date().getTime(),
							type: "post",
							dataType: "json",
							timeout: 600000
						});
		...
		_close: function (e) {
			...
			var compareElement = function (ele, element) {
				return !(ele === element || $.contains(element, ele));
			};

			if (this.isShowSidebar && this.$sidebar) {
				if (compareElement(currEle, this.$sidebar[0])) {
					this.$sidebar.close();
				}
			}
